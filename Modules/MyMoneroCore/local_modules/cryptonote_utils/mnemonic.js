...
(function(exports)
{
	var isNotToUseWindowCrypto
	if (typeof window !== 'undefined' && window.IsElectronRendererProcess !== true && window.IsJavascriptCore !== true) {
		isNotToUseWindowCrypto = false
	} else {
		isNotToUseWindowCrypto = true
	}
	//
	var crc32 = require('./crc32').crc32
	var required__crypto;
	if (isNotToUseWindowCrypto == true) {
		required__crypto = require('crypto')
	}
	//
	...
	function mn_random(bits) {
		...
		if (isNotToUseWindowCrypto == false) {
			...
			//			v---- Declared above
			const buffer = required__crypto.randomBytes(bits / 8) // assuming 8 bits in byte
