...
func DoesRecordHaveValidOpenAliasPrefix(_ record: String, _ openAliasPrefix: String) -> Bool
{
...
func New_ParsedDescriptionFromOpenAliasRecordWithOpenAliasPrefix(
	record: String,
...
func ValidatedOARecordsFromTXTRecordsWithOpenAliasPrefix(
	domain: String,
	records: [String],
	dnssec_used: Bool,
	secured: Bool,
...
) -> (err_str: String?, validated_descriptions: [ParsedOARecipientDescription]?)
{
	...
	for (_, record) in records.enumerated() {
		let (err_str, parsed_description) = New_ParsedDescriptionFromOpenAliasRecordWithOpenAliasPrefix(
			record: record,
