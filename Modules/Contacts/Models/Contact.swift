...
class Contact: PersistableObject
{
	...
	var hasOpenAliasAddress: Bool {
		return MyMoneroCoreUtils.doesStringContainPeriodChar_excludingAsXMRAddress_qualifyingAsPossibleOAAddress(self.address)
	}
	...
	) -> String? // err_str -- maybe port to 'throws'
	{
		...
		self.emoji = emoji
		self.address = address
		if MyMoneroCoreUtils.doesStringContainPeriodChar_excludingAsXMRAddress_qualifyingAsPossibleOAAddress(address) == false {
